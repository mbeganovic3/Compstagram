...
import { filterAboveCarriage } from "../ps02-hogwarts-express/index-app";
    ...
    process = (input: Image): Image => {
        ...
        for (let row: number = 0; row < output.height; row++) {
            for (let col: number = 0; col < output.width; col++) {
                ...
                } else if (col >= input.width - thickness) {
                    output.pixels[row][col] = this.color;
                } else if (row < thickness) {
                    output.pixels[row][col] = this.color;
                } else if (row >= input.height - thickness) {
                    output.pixels[row][col] = this.color;
                }
            }
        } 
...
export class BrightnessFilter extends Filter {
    ...
    process = (input: Image): Image => {
        // TODO 2.2
        let output: Image = input.copy();
        let brightness: number = (this.amount - 0.5) * 2;
        ...
        for (let row: number = 0; row < output.height; row++) {
            for (let col: number = 0; col < output.width; col++) {
                ...
                pixel.red = red + (brightness * red);
                pixel.green = green + (brightness * green);
                pixel.blue = blue + (brightness * blue);
            }
        }
    ...
    process = (input: Image): Image => {
        ...
        for (let row: number = 0; row < output.height; row++) {
            for (let col: number = 0; col < output.width; col++) {
                 ...
                 let redDelta: number = (average - red) * contrast;
                 let greenDelta: number = (average - green) * contrast;
                 let blueDelta: number = (average - blue) * contrast;
